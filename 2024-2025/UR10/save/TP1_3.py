...
def init():
    global tf, xf, Rf, start_time, q, q_graph, t_graph
    start_time = time.time()  # Récupération du temps de démarrage
    tf = 5  # Temps final de la simulation
    xf = np.array([1, 1, 1])  # Position finale
    q = np.zeros([6,1])       # Valeur tampon
...
def update():
    ...
    while True:
        ...
        T06, J = kinematics(q)  # Utilisation de positions de jointure fictives (zeros)
        ...
        q = Correcteur_et_Controle(T06[:3, :3], rd_point, theta, np.array([ux, uy, uz]), J, xd_point, e0, ep, L)
        ...
        if i == 0:
            ...
            q_graph = np.hstack([q_graph, q])



def Graphique():
    global q_graph, t_graph

    fig, axs = plt.subplots(6, 1, figsize=(8, 10))  # 6 lignes, 1 colonne de subplots
    couleurs = ['b', 'g', 'r', 'c', 'm', 'y']

    print(q_graph[0, :].shape)
    print(t_graph.shape)

    for i in range(6):
        axs[i].plot(t_graph, q_graph[i, :], color=couleurs[i], label=f'q{i+1}')  # Tracer en fonction de t_graph
        axs[i].legend(loc='upper right')  # Légende dans chaque subplot
        axs[i].grid(True, which='both', alpha=0.5)  # Grille avec opacité réduite
        axs[i].minorticks_on()            # Active les minor ticks
        axs[i].tick_params(which='both', direction='in')  # Définit l'apparence des ticks
        axs[i].set_ylabel(f'q{i+1}')
        if i == 5:
