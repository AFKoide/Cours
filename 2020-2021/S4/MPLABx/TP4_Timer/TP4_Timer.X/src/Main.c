...
*/
    if(S3)
    {
        //TODO: Lancer le chronomètre et l'arrête
        if (!EdgeS3)
        {
            EdgeS3 = 1;
            if(ChronoStatus == GO)
            ...
            else if(ChronoStatus == STOP || ChronoStatus == RTCC)
                ChronoStatus = GO;
        }
    }
    else
    ...
    if(S4)
    {
        //TODO: Affiche l'heure réelle
        EdgeS4 = 1;
        if(!EdgeS4)
        {
            if(ChronoStatus == GO)
                ChronoStatus = RTCC;
            else if (ChronoStatus == RTCC)
                ChronoStatus = GO;
        }
    }
    else
    ...
    if(S5)
    {
        //TODO: Réinitialiser le compteur
        ChronoTime.count = 0;
        InterTime.count = 0;
        ChronoTime.second = 0;
        InterTime.second = 0;
...
{   
    ...
    LcdToDoL1 = ON;
    if (*Stat == INTER)
    {   sprintf (LcdBuffL2, "Temps Tour %2d:%02d", InterTime.minute, InterTime.second);
        LcdToDoL2 = ON;
        *Stat = GO;
    }
    */
...
{ 
    ...
    m = RtccTime.minute;
    s = RtccTime.second;
    sprintf (LcdBuffL1, "%d/%02d/%02d %2d:%02d:%02d", D, M, Y, h, m, s);
    LcdToDoL1 = ON;
    */
